...
<section class="wrap flex <?= ($side == 'left' ? 'flex-col-reverse md:flex-row' : 'flex-col-reverse md:flex-row-reverse'); ?> justify-center gap-3 lg:gap-12">
    <article class="w-full md:w-3/5 xl:w-5/12">
        <?php if ($title) : ?><h1 class="font-bold text-2xl md:text-3xl mb-4 md:mb-5 bg-clip-text text-transparent bg-gradient-to-r from-yellow-200 via-yellow-400 to-yellow-700"><?= esc_html($title); ?></h1><?php endif; ?>
        <?php if ($text) : ?><p class="text-base md:text-lg text-white"><?= esc_html($text); ?></p><?php endif; ?>
    </article>
    <?php if ($img || $vid) : ?>
        <div class="w-1/2 sm:w-1/3 md:w-2/5 lg:w-1/3 xl:w-3/12 realtive rounded-full overflow-hidden aspect-square border-8 border-transparent  flex items-center justify-center">
            <?php if ($img) : ?>
